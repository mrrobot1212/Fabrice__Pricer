...
def func1_365_noncomp():
    fields = ('Loan Principle, $','Annual Rate, %', 'Number of Annual Periods',  'Years Until Maturity', 'Total Payable, $', 'Maturity Date')
    ...
    def final_balance(entries):
        ...
        r = float(entries['Annual Rate, %'].get()) 
        loan = float(entries['Loan Principle, $'].get())
        n =  float(entries['Number of Annual Periods'].get()) 
        y = float(entries['Years Until Maturity'].get())
        

        fin= int((loan * (1 + (r/100)) - loan) * (n*y))
        fin= int(fin + loan)
        fin_res = fin 
        fin = (f"{fin:,d}")
        entries['Total Payable, $'].delete(0, tk.END)
...
def opt_win():
    ...
    y_365.add_command(label='Non-Compounding Interest Calc', command=None) #func4_365_
    ...
    y_360.add_command(label='Non-Compounding Interest Calc', command=None) #func4_360_



    non_comp = Menu(menubar, tearoff=0)
    menubar.add_cascade(label='Non-Comp', menu=non_comp)
    non_comp.add_command(label='Interest Over Fixed Period -- 365', command=func1_365_noncomp)
    non_comp.add_command(label='Interest Over Fixed Period -- 360', command=func1_360_noncomp)
    non_comp.add_command(label='Interest for Used and Unused -- 365', command=func2_365_noncomp)
    non_comp.add_command(label='Interest for Used and Unused -- 360', command=func2_360_noncomp)
